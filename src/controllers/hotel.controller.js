...
export const defaultHotel = async(req, res) => {
    try{
        const data = {
            ...
            description: 'Este hotel de ciudad dispone de 397 elegantes habitaciones equipadas y reformadas completamente. En el año 2013 se realizó la instalación, en todas las habitaciones, de dispositivos de ahorro de energía como iniciativa para preservar el medio ambiente. Además, cuenta con habitaciones de categoría Premium para los huéspedes que estén interesados en disfrutar de una estancia con un plus en servicios y equipamiento.'
        }
...
export const defaultHotel2 = async(req, res) => {
    try{
        const data = {
            ...
            country: 'Guatemala',
            address: 'The Westin Camino Real, Guatemala Avenida la Reforma y 14 Calle, Ciudad de Guatemala',
            description: 'Situado en el distrito más moderno y exclusivo de Guatemala, a pocos minutos de tiendas locales, restaurantes de primer nivel, centros comerciales y lugares de entretenimiento.'
        }
        let defualtCreated = await Hotel.findOne({name: data.name})
        if(!defualtCreated){
            let hotel = new Hotel(data)
...
export const defaultHotel3 = async(req, res) => {
    try{
        const data = {
            ...
            description: 'Descubra la Ciudad de Guatemala durante su estadía en la Zona 10, una de las más atractivas de la ciudad con museos, cafés, restaurantes y la impresionante arquitectura del siglo XIX. Dentro de la Zona 10, uno de los distritos más animados de la ciudad, con hermosas calles bordeadas de árboles y mucho más.'
        }
        let defualtCreated = await Hotel.findOne({name: data.name})
        if(!defualtCreated){
            let hotel = new Hotel(data)
...
export const defaultHotel4 = async(req, res) => {
    try{
        const data = {
            ...
            description: 'At Hoteles Citadin we are proud to offer decent and clean rooms at affordable prices, in demanded locations of Guatemala. We know life is out there and not inside a hotel, so we focus on what is most important for you.'
        }
        let defualtCreated = await Hotel.findOne({name: data.name})
        if(!defualtCreated){
            let hotel = new Hotel(data)
...
export const defaultHotel6 = async(req, res) => {
    try{
        const data = {
            ...
            description: 'Conquistador Hotel & Conference Center posee una ubicación privilegiada a 15 minutos del aeropuerto. Rodeado de centros de financieros, comerciales, museos, teatros y el Centro Histórico hacen que sea un lugar ideal para su estancia en viajes de negocios o turísticos.'
        }
        let defualtCreated = await Hotel.findOne({name: data.name})
        if(!defualtCreated){
            let hotel = new Hotel(data)
